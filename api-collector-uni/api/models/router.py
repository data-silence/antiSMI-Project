...
from api.models.services import get_universal_model, get_headline_model, get_summary_model
...
@router.post('/generate_embs')
async def make_emb(query: list[str], universalizer=Depends(get_universal_model)) -> list[list[float]]:
    """Handler to fetch embeddings for query"""
    return await universalizer.get_embeddings(texts=query)


@router.post('/get_category')
async def get_category(text: list[str], universalizer=Depends(get_universal_model)) -> list[str]:
    return await universalizer.predict_categories(text)
